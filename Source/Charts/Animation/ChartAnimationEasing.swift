...
internal struct EasingFunctions
{
    ...
    internal static let EaseOutBack: (TimeInterval, TimeInterval) -> Double = { elapsed, duration in
        let s: TimeInterval = 1.70158
        var position: TimeInterval = elapsed / duration
        position -= 1.0
        let x: Double = (s + 1.0) * position + s
        return Double( position * position * x + 1.0 )
    }
